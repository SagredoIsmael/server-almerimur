...
try {
  ...
  $query = "SELECT mechanic_work_date FROM mechanic_work WHERE mechanic_work_user_id='$user_id' ORDER BY mechanic_work_date";

  $result = api_get($query);

  $response = [
    'message' => "Fechas de trabajos obtenidas",
    'dates' => $result
  ];
